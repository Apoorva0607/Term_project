...
# Input = ['2 -0.0851079674400284875 -2.6567444497953426E7 189.1449978825454 270.12705171119865\n',
# '5 -0.0704728084335385951 -1.8308219249732424E7 -1.5243752053503457E7 1.1758666527103167E7\n',
# '8 -0.0676069465542134112 -6472095.642237772 -1.6502711754240574E7 1.978892594394039E7\n',
# '15 -0.0682503846845445311 -3749459.7822091323 -1.5085939791716123E7 2.1544954843370933E7\n',
# '18 -0.0689914381590615142 1304433.7451257461 -2.0805463350272946E7 1.646998040817312E7\n',
# '21 -0.0800564989993973894 1.6481439642660044E7 -1.990188225199734E7 6173033.078930248\n',
# '2 0.914890473832339394 -2.6567444263503283E7 -2033.2638814814845 -2903.801759518517\n',
# '5 0.929526589361812113 -1.830764627583926E7 -1.5246499972803518E7 1.1755995648120636E7\n',
# '8 0.932393307554027837 -6469533.030457307 -1.6505300372969607E7 1.9787604939114068E7\n',
# '15 0.931749147584779588 -3745623.867544954 -1.5086253279878343E7 2.1545402550864942E7\n',
# '18 0.931008363081359831 1307157.9532807998 -2.0803650002613228E7 1.6472054878324244E7\n',
# '21 0.919944791247471286 1.6482735558696747E7 -1.989986466584745E7 6176076.5898432415\n']

def deg2radians(a1,a2,a3,Pi):
    return (Pi*(int(a1) + (int(a2)/60) + float(a3)/(60*60)))/180
#d = deg2radians('111','48','40.0',Pi)
#print(d)
...
def PositionInCartesian_1(split,R,Pi,s):
    ...
    X1 = np.matmul(R3_alpha,X)
    return t,X1
#a = "12123.0 40 45 55 1 111 50 58 -1 1372"
#tv,Xv = PositionInCartesian(a,R,Pi,s)
#print(tv)
...
def CalculateBounds(Input, aI):
  counter = 1
  while aI < len(Input) and (int(Input[counter].split()[0]) - int(Input[counter - 1].split()[0])) > 0:
  
    counter = counter + 1
  return counter 

#Bound = CalculateBounds(Input, 8)
#print(Bound)
...
#SatelliteDat1,aI = ReadData(Input, 0)
#print(SatelliteDat1[0][0])
...
#b12 = '0.0 40 45 55.0 1 111 50 58.0 -1 1372.0'
#[t0,X0] = PositionInCartesian(b12,R,Pi,s)
#gradF = F(X0, c, SatelliteDat1)
...
#B = HorizonCheckReciever(t0, X0, SatelliteDat1)
...
#Jac1 = Jacobian(X0, c, SatelliteDat1)
#print(Jac1)
...
#retVal = NewtonsMethod(X0,0,R,Pi,c,SatelliteDat1)
#print(retVal)

def TimeAT(Xv,SatelliteDat1,c):
  Tv1 = SatelliteDat1[0][0] + (1/c) * (math.sqrt(math.pow(Xv[0]-SatelliteDat1[0][1], 2)+math.pow(Xv[1]-SatelliteDat1[0][2], 2)+math.pow(Xv[2]-SatelliteDat1[0][3], 2)))
  if Tv1 < 0.01/c:
    Tv1 = 0.0
  return Tv1

#Tv = TimeAT(retVal,SatelliteDat1,c)
#print(Tv)
...
def RadiansToDegrees(a,Pi):
  ...
  if minutes == 60:
    minutes = 0
    minutes = minutes + 1
  if degrees < 0 or degrees > 180 or minutes < 0 or minutes > 59 or seconds < 0.0 or seconds >=60.0 or a < -Pi or a > Pi:
    print('angles error')
  
  return degrees, minutes, seconds, plus

#degrees, minutes, seconds, plus = RadiansToDegrees(-1.9521410720820356,Pi)
...
def PositionInGeographic(XV,TV,R, Pi, c, s):
  ...
  return h, Psi, Lambda
#h, Psi, Lambda = PositionInGeographic(X0, t0, R, Pi, c, s)
#print(Psi)
#print(Lambda)
...
#[TV, Psi, Lambda, h] = ComputeLocation(X0, SatelliteDat1,R, Pi, c, s)
#print(TV, Psi, Lambda, h)

"""The test was to see if your run cat b12t0.dat | java vehicle you get

0.0 40 45 55.0 1 111 50 58.0 -1 1372.0

1.0 40 45 55.0 1 111 50 58.0 -1 1372.0

In this program i take the satellite data (Input) and try to find the these two lines.

Here my test was to convert 40 45 55.0 1 111 50 58.0 -1 1372.0 to cartesian and see if it matches the output of reciever program and it does. Reciever program will only read satellite output - which is variable Input here and data.dat file
...
# test
#aI = 0
#b12 = '12123 40 45 55.0 1 111 50 58.0 -1 1372.0' # starting point 
#t0,X0 = PositionInCartesian(b12,R,Pi,s)
#while aI < len(Input):
#	data, aI = ReadData(Input, aI)
  #B = HorizonCheckReciever(t0, X0, data)
#	[TV, Psi, Lambda, h] = ComputeLocation(X0,data,R, Pi, c, s)
#	print(TV,Psi, Lambda, h)
...
while aI < len(Input):
  ...
  a1 = [str(start_time)] + b12_dash
  tv1,Xv1= PositionInCartesian_1(a1,R,Pi,s)
  B = HorizonCheckReciever(tv1, Xv1, data)
  if all(B) == True:
    print("All satellites are in horizon")
  TV, Psi, Lambda, h = ComputeLocation(Xv1, tv1, data, R, Pi, c, s)
  print(TV, Psi, Lambda, h)
